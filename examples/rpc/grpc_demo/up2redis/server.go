...
func (s *Server) Start() error {
	...
	li, err := net.Listen("tcp", Listen)
	log.Println("Grpc listening ", Listen)
	if err != nil {
		log.Fatalf("Failed to listen: %s", err.Error())
	}

	rs := grpc.NewServer()
	redis.RegisterRedisServer(rs, s)
...
func (s *Server) Report(ctx context.Context, in *redis.ReportRequest) (*redis.ReportResponse, error) {
	...
	return &redis.ReportResponse{Info: "push data success"}, nil
}

func (s *Server) uploader(in *redis.ReportRequest, pool *Pool) {
	if in.Delay == 0.0 {
